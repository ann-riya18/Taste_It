...
// Get current week's session key
$year = date('Y');
$month = date('n');
$selected_week = isset($_GET['week']) ? (int)$_GET['week'] : ceil(date('j') / 7);
if ($selected_week > 4) $selected_week = 4;
$session_key = "meal_plan_{$year}_{$month}_week{$selected_week}";

// Check if meal plan exists in session
if (!isset($_SESSION[$session_key]) || empty($_SESSION[$session_key])) {
    header("Location: what_to_cook.php");
    exit();
}
...
if (isset($_GET['swap_day']) && isset($_GET['swap_course'])) {
    ...
    // Get current recipe ID for this meal
    ...
    // Add current recipe to excluded list
    $existing_recipe_ids[] = $current_recipe_id;
    
    // Create placeholders for prepared statement
    $placeholders = implode(',', array_fill(0, count($existing_recipe_ids), '?'));
    
    // Find a random recipe of the same course type
    ...
    $params = array_merge([$swap_course . '%'], $existing_recipe_ids);
    ...
    header("Location: shuffle_meal.php?week=$selected_week");
    exit();
}
...
<body>
  ...
  <div class="container">
    <a href="what_to_cook.php" class="back-btn">
      <i class="fas fa-arrow-left"></i> Back to Meal Plan
    </a>
    
    <div class="header">
      <h2><i class="fas fa-random"></i> Shuffle Meal Plan</h2>
      <p>Click swap to replace a meal</p>
    </div>
    
    <?php if (isset($_SESSION['swap_message'])): ?>
      <div class="message">
          ...
          <?php foreach ($meals as $course => $meal): ?>
            <?php if (isset($meal['id']) && $meal['id'] > 0): ?>
              <div class="meal-item">
                ...
                <a href="?week=<?php echo $selected_week; ?>&swap_day=<?php echo $day; ?>&swap_course=<?php echo $course; ?>" class="swap-btn">
